...
for dfv in DFV:
    ...
    if runMany:
        for h in [i for i in [0,6*24]]: # Hour-ahead Smart Charging
            print("h = " + str(h))
    ...
    ### DumbCharge
    prob, x, b = DumbCharge(b0, bmax, bmin_within, xmax, c_within, c_tilde, u2_within, z_within, T_within, tvec_within, r=r, verbose=False)
    if LpStatus[prob.status] == 'Optimal':
    ...
    # Kommentarer til DumbCharge:
    # - Kun tre kører på u_within
    # - Flere kører på u2_within
    # - Vehicle24727 vil ikke kører på u2, men gerne på u
